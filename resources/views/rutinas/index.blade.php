<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('rutinas.create') }}" class="btn_green">+ Nueva Rutima</a>
        </div>
    </x-slot>
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <form method="GET" action="{{ route('rutinas.index') }}" class="mb-4">
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-gray-800 dark:text-gray-100">
                        @foreach($rutinas as $rutina)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td class="px-4 py-2">{{ $rutina->nombre }}</td>
                                <td class="px-4 py-2 space-x-2">
                                    <a href="{{ route('rutinas.edit', $rutina->id) }}" class="text-indigo-600 hover:underline">✏️ Editar</a>
                                    <form action="{{ route('rutinas.destroy', $rutina->id) }}" method="POST" class="inline"
        ...
        @else
            <p class="text-gray-500 dark:text-gray-300 mt-4">No hay rutinas cargadas aún.</p>
        @endif
    </div>
    <div class="mt-4">
        {{ $rutinas->links() }}
