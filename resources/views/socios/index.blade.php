<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('socios.create') }}" class="btn_green">+ Nuevo Socio</a>
        </div>
    </x-slot>
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-gray-800 dark:text-gray-100">
                        @foreach($socios as $socio)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-2">{{ $socio->direccion }}</td>
                                <td class="px-4 py-2">{{ $socio->localidad }}</td>
                                <td class="px-4 py-2">{{ $socio->telefono }}</td>
                                <td class="px-4 py-2">
                                    @if($socio->activo)
                                ...
                                <td class="px-4 py-2 space-x-2">
                                    <a href="{{ route('socios.edit', $socio->id) }}" class="text-indigo-600 hover:underline">✏️ Editar</a>
        ...
        @else
            <p class="text-gray-500 dark:text-gray-300 mt-4">No hay socios cargados aún.</p>
        @endif
    </div>
    <div class="mt-4">
        {{ $socios->links() }}
