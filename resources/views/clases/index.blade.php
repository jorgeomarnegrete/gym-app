<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('clases.create') }}" class="btn_green">+ Nueva Clase</a>
        </div>
    </x-slot>
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <form method="GET" action="{{ route('clases.index') }}" class="mb-4">
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <table class="min-w-full text-sm divide-y divide-gray-200">
                    <thead class="bg-gray-100 dark:bg-gray-700 text-left text-xs uppercase tracking-wide text-gray-600 dark:text-gray-300">
                        <tr>
                            ...
                            <th class="px-4 py-3">Tipo Costo</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-gray-800 dark:text-gray-100">
                        @foreach($clases as $clase)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-2">{{ $clase->tipo_costo }}</td>
                                <td class="px-4 py-2">{{ $clase->costo }}</td>
                                <td class="px-4 py-2">
                                    @if($clase->activo)
                                ...
                                <td class="px-4 py-2 space-x-2">
                                    <a href="{{ route('clases.edit', $clase->id) }}" class="text-indigo-600 hover:underline">✏️ Editar</a>
        ...
        @else
            <p class="text-gray-500 dark:text-gray-300 mt-4">No hay clases cargadas aún.</p>
        @endif
    </div>
    <div class="mt-4">
        {{ $clases->links() }}
